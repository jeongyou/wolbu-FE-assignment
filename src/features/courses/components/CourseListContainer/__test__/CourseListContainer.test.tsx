...
const renderContainer = () => {
  ...
  return render(
    <CourseListContainer initialPage={initialPage} initialParams={initialParams} />
  );
};
...
describe('CourseListContainer', () => {
  it('선택이 없으면 수강 신청 버튼이 비활성화되고, 선택 시 활성화된다', () => {
    ...
    const enabledButtons = screen.getAllByRole('button', { name: /수강 신청 하기/i });
    const enabledButton = enabledButtons.find((btn) => !btn.hasAttribute('disabled'));
