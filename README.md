...
- 강의 목록 페이지는 **Server Component 기반 SSR**로 구현
- 첫 화면 로딩 성능을 고려해 SSR 적용

---

### 2. 페이지 특성에 따른 SSR / CSR 분리

- 무한 스크롤, 정렬 변경, 필터링 등 상호작용이 많은 영역은 **Client Component**에서 처리
- 강의 개설, 회원가입 등 나머지 페이지는 CSR로 구성해 불필요한 SSR 복잡도를 줄임
- 페이지 역할에 따라 렌더링 방식을 분리해 구조를 단순하게 유지

---

### 3. 무한 스크롤 구조 설계

- `IntersectionObserver`를 사용해 하단 센티널이 화면에 진입하면 다음 페이지를 요청
- 스크롤 이벤트 기반 구현 대비 불필요한 이벤트 호출을 자연스럽게 방지
- 데이터 로딩 여부를 기준으로 중복 요청을 차단해 안정적인 페이지 병합을 보장
...
- `AppWrapper`로 전체 컨테이너를 감싸고,
  ...
  툴바, 카드 목록, 폼 필드 정렬까지 일관된 레이아웃을 유지

---

### 5. 인증 및 권한 관리
- 인증 및 권한 관리는 localStorage 기반으로 구현
...
- 불필요한 API 호출을 줄이기 위해 클라이언트 정렬 방식도 고려
- 하지만 무한 스크롤 특성상 전체 데이터를 보유하지 않기 때문에,
  일부 데이터만 정렬할 경우 새 페이지가 로드되면서 정렬 순서가 깨지는 문제 발생
- 서버 정렬은 전체 데이터를 한번에 내려 받는게 아니라 정렬된 데이터를 페이지네이션 형태로 내려받는 구조
- 이에 따라 정렬 기준을 서버로 전달해 **서버 정렬 방식**으로 구현

## 🛠 구현 이슈 및 해결

### 하이드레이션 불일치 이슈

- 강의 개설 버튼(CreateCourseButton)은 사용자 권한(localStorage 기반)에 따라 조건부 렌더링
- 서버 렌더링 시 사용자 정보를 알 수 없어,
  서버 HTML과 클라이언트 첫 렌더 결과가 달라지는 하이드레이션 불일치 문제 발생

#### 해결 방법

- 해당 버튼 컴포넌트를 CSR 전용으로 분리
- `dynamic import`와 `ssr: false` 옵션을 적용해 클라이언트에서만 렌더링되도록 처리
이를 통해 권한 기반 UI를 하이드레이션 오류 없이 안정적으로 제공 가능



## 🧪 테스트 코드 작성 범위

핵심 비즈니스 로직과 사용자 흐름을 중심으로 테스트 코드를 작성
...
- **Emotion**  
  - 컴포넌트 단위로 스타일을 분리하고 캡슐화하기 위해 사용
  - props 기반 **동적 스타일링 가능**
  - `*.styles.ts` 구조로 로직과 스타일을 분리

- **react-hook-form**  
  - 폼 상태, 입력값 검증, 에러 처리를 선언적으로 관리하기 위해 사용 
  - 회원가입과 강의 개설 폼에서 필드 단위 검증과 제출 제어를 간결하게 구현

- **react-icons**  
  - SVG 아이콘을 컴포넌트 형태로 사용해 UI 구성 속도를 높이기 위해 사용

- **Vitest**  
  - 빠른 실행 속도의 테스트 러너, 도메인 로직과 훅 테스트를 중심으로 사용

- **Testing Library**  
  - 사용자 관점에서 UI 동작을 검증하기 위해 사용 
  - 폼 입력, 버튼 활성화, 에러 노출 등 주요 사용자 흐름을 테스트
